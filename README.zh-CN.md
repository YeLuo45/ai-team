...
| 🌊 **Reveal 滚动入场 + PanelStatus 状态点** | Reveal（5 方向 + 4 delay + useReveal hook + IntersectionObserver + RevealList stagger）+ PanelStatusIndicator（StatusDot + count + Badge + 4 状态点色）+ PanelStatusGrid（2x4 网格）+ usePanelStatus hook + statusPriority | V141-V142 |
| 📄 **Interview 详情 — 简历查看 + 多轮次分组** | groupInterviewsByCandidate/sortRoundsByTime/buildRoundLabel/extractResumeSections/summarizeResume/formatRoundTimeline 9 个纯函数 + ResumeCard（GlassCard + 预览 + 可折叠完整简历 4 段）+ RoundTabs（一面/二面/三面/四面 tablist）+ CandidateInterviewPanel（resume + rounds tabs + per-round detail）+ Interviews 页改造为候选人分组侧栏 + 每候选人详情面板 + 44-test v143 套件 + v143/web-interview-helpers strict layer 100%/96.39% | V143 |
| 📈 **Interview 多轮 sparkline + 相对时间 + seed 多轮闭环** | RoundsComparison（SVG 折线对比 overall/4 breakdown + 趋势标签 ↑↓→）+ 6 个纯函数（buildRoundsSparkline/scoreToY/buildSparklinePath/buildSparklineX/SPARKLINE_METRICS/RoundsSparklinePoint）+ 候选人卡片相对时间（X 小时/天前）+ seed 4 位示例候选人（前端/后端/产品/算法 × 2-4 轮面试）+ 11 个新面试（3→14）+ 19-test v144 套件 + v144 strict layer | V144 |
| 🔗 **Candidates → Interview 详情 deep-link** | Candidates 卡片底部「N 场面试」计数 + 「📋 查看面试详情」按钮（source=static 也可点）+ useNavigate 跳转 /interviews?candidate=<id> + Interviews useSearchParams auto-select + 手动切换同步 URL + 候选人不存在 fallback + 4-test v145 套件 + 现有 Interview 测试 MemoryRouter 包裹 | V145 |
| ↔️ **Interview 详情跨候选人导航 (返回/上一个/下一个)** | CandidateInterviewPanel 新增 NavToolbar（← 返回候选人列表 + ← 上一个 / 下一个 → + N/Total 位置 + 边界禁用 + tooltip 显示上/下一位姓名）+ CandidateNavContext 类型 export + Interviews useMemo 计算 navContext + 全局键盘快捷键 ←/→（输入框中不触发）+ navContext fallback 到 groups[0] 避免闪烁 + 8-test v146 套件 | V146 |
| 🔀 **Interview 对比模式 — 同岗位候选人 sparkline 矩阵** | 新组件 ComparisonMatrix（按 position 分组 + 每组 SVG 候选人 × 轮次矩阵 + 🏆 Top scorer + onSelectCandidate 切换单候选人）+ 4 个纯函数 helpers（buildCandidateComparisonRow/groupComparisonByPosition/CandidateComparisonRow/PositionComparisonGroup）+ Interviews 页 🔀 对比模式 toggle + URL ?compare=1 同步 + 16-test v147 套件 + v147 strict layer | V147 |
| ☑️ **Candidates 多选 + 批量操作** | Candidates 卡片顶部 checkbox（仅 api 模式）+ 顶部「全选当前 / 取消全选」toggle + sticky batch-action-toolbar（已选 N 位 + 取消选择 + 🗑 批量删除 (N)）+ toggleSelect/clearSelection/selectAllVisible/handleBatchDelete 6 个 handlers + filter 变化时清理不可见 ID + 6-test v148 套件 | V148 |
| 🔄 **Comparison matrix 维度切换 + Pipeline 招聘流程进度条** | ComparisonMatrix 加 metric switcher (overall/technical/communication/problemSolving/culture 5 个 radio) + bestByMetric/avgByMetric per-metric stats + metricSeries helper + per-metric top scorer 重新计算 + 10-test v149 套件 + 新增 PipelineProgress 5 阶段 timeline (新录入→筛选中→面试中→Offer→已入职) + current 高亮 + rejected off-path badge + mapStatusToPipeline helper + 9-test v150 套件 | V149-V150 |
| 📤 **Candidates 批量改状态 + 批量导出简历** | batch-action-toolbar 扩展「改状态为」dropdown（6 状态）+ handleBatchUpdateStatus（confirm + Promise.all + refresh）+ 「📤 导出简历」按钮（Blob + createObjectURL + <a>.click()）+ 新 lib/resume-export.ts（buildResumeJsonExport/serializeResumeExport/buildResumeExportFilename）+ CANDIDATE_STATUSES 常量 + 8-test v151 套件 | V151 |
| ⏩ **Pipeline 上一/下一阶段快捷操作** | PipelineProgress 增 onAdvance/busy props + ← 上一阶段 / 下一阶段 → 按钮（边界禁用 + tooltip）+ 3 个 helpers（nextStage/prevStage/stageToStatus）+ CandidateInterviewPanel 增 pipeline?: PipelineAdvanceHandler prop + Interviews 页 handlePipelineAdvance（api.updateCandidate + refresh + pipelineBusy state）+ 16-test v152 套件 | V152 |
| 📝 **RejectReasonModal — Pipeline 被拒原因记录** | 新组件 RejectReasonModal（候选人姓名 + 4-row textarea + char counter REJECT_REASON_MIN/MAX=4/500 + 5 个 suggestion chips + 关闭重开 reset + busy 禁用）+ PipelineProgress 增 onRecordReject prop（仅 status='rejected' 时显示「📝 记录被拒原因」按钮）+ Interviews 页 handleRejectSubmit（拼接 [rejected ISO timestamp] reason 到 candidate.notes + api.updateCandidate + refresh）+ 14-test v153 套件 | V153 |
| 🔄 **Pipeline off-path 恢复按钮** | PipelineProgress 增 onRestore?: (next) => void prop + 「🔄 恢复为面试中」按钮（仅 status='rejected' + onRestore 存在时渲染 + busy 禁用）+ CandidateInterviewPanel PipelineAdvanceHandler 增 onRestore? 字段 + Interviews 页 handleRestoreFromReject（api.updateCandidate(status: 'interviewing') + 不修改 notes 保留审计 + setPipelineBusy + refresh）+ 6-test v154 套件 | V154 |
| 🔗 **Comparison Matrix URL ?metric= 持久化** | ComparisonMatrix 改 controlled mode（metric?/onMetricChange? props + 受控时 prop 优先）+ isValidMetricKey type guard + METRIC_OPTIONS export + Interviews 页从 searchParams.get('metric') 读取 + handleMetricChange 同步 URL + 9-test v155 套件 | V155 |
| 📜 **RejectHistoryList — 候选人历史被拒原因时间线** | 新组件 RejectHistoryList（parseRejectNotes 解析 [rejected <iso>] <reason> 行 + formatRejectTimestamp YYYY-MM-DD HH:MM + RejectEntry 类型）+ 按时间倒序展示 + 数量徽章 + maxItems 截断（默认 5）+ CandidateInterviewPanel 当 status='rejected' 时自动展示 + 12-test v156 套件 | V156 |
| 💾 **ResumeCard 折叠状态持久化到 localStorage** | ResumeCard 增 readExpandedFromStorage/writeExpandedToStorage SSR-safe helpers + useState lazy init 从 localStorage 读取 + useEffect 同步变化到 storage + per-candidate key 'ai-team:resume-card:expanded:<id>' + try/catch 捕获 quota 错误 + 9-test v157 套件 | V157 |
| 💡 **ComparisonMatrix metric tooltip 描述** | 新增 METRIC_DESCRIPTIONS: Record<ComparisonMetricKey, string> (overall/technical/communication/problemSolving/culture 5 个 metric 含义) + ComparisonMatrix metric 按钮加 title attribute + 右侧新增 inline description label 实时跟随当前 metric + 4-test v158 套件 | V158 |
| ⏱ **PipelineProgress 时间停留统计** | 新增 computeTimeInCurrentStage helper（输入 updatedAt + 可选 now Date，返回 { days, hours, formatted, since }，6 种格式化 'X 天 Y 小时' / 'X 天' / 'X 小时 Y 分钟' / 'X 分钟' / '刚刚' / '—'）+ PipelineProgress 新增 stageEnteredAt prop + UI '⏱ 在 X 阶段停留 Y' + CandidateInterviewPanel 接入 + 12-test v159 套件 | V159 |
| 📥 **ComparisonMatrix 导出 CSV 按钮** | 新增 buildComparisonCsv helper（列: 岗位/候选人/候选人ID/已评估轮次/最高分(metric)/平均分(metric) + RFC-4180 escape）+ buildComparisonCsvFilename（comparison-<metric>-YYYY-MM-DD.csv）+ handleExportCsv（Blob + createObjectURL + <a>.click() + 1s revokeObjectURL）+ 「📥 导出 CSV」按钮 + 6-test v160 套件 | V160 |
| 🎤 **STT Provider 抽象层 + 切换 UI (V161 - 实时面试指导第一波)** | packages/ai-team-web/src/lib/stt/types.ts (SttProvider/SttSession/SttTranscriptChunk/SttState 接口) + WebSpeechProvider（浏览器 SpeechRecognition + webkit fallback）+ MockSttProvider（预制脚本循环 + 可配置 intervalMs）+ WhisperSttProvider stub + registry.ts (listSttProviders/Options/getSttProvider/getDefaultSttProviderId) + SttSettings 组件（provider dropdown + 🔒 本地/☁️ 远程 badge + 状态 badge + 开始/停止按钮 + 200 段 transcript 流 + 自动 fallback + onBufferChange callback）+ CandidateInterviewPanel 接入 + 14-test v161 套件 | V161 |
| 🧠 **QuestionSuggestionAgent 接口 + Mock/LLM (V163 - 实时面试指导第三波)** | packages/ai-team-web/src/lib/question-suggestion/types.ts (QuestionSuggestionAgent / QuestionSuggestion / SuggestionTrigger 接口) + MockQuestionSuggestionAgent（8 个 template pool + deterministic + 自动避开 focusTag）+ LlmQuestionSuggestionAgent（自定义 system prompt + JSON parse + ```json fence 支持 + LlmClient 接口解耦）+ registry.ts (listQuestionSuggestionAgents/Options/get/default 'mock') + 13-test v163 套件 | V163 |
| 🎨 **Hero 大屏 + Glass surface（参考 Tencent 混元）** | HeroLanding（eyebrow + title + subtitle + 4 模块矩阵 + CTA）+ StatusDot（4 状态点语义色）+ HeroBackground（orbs/grid/plain 3 变体）+ buildDefaultHeroModules（recruitment/orchestration/audit/data）+ GlassCard（backdrop-blur + 半透明 + 4 主题 token）+ TopbarGlass（sticky + glass surface + CTA） | V139-V140 |
